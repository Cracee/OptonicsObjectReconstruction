...
from learning3d.data_utils import RegistrationData, ModelNet40Data, RegistrationDataFragments, StanfordFragments, SyntheticFragments
...
def display_open3d(template, source, transformed_source, extra_data=None, all_it_score=None):
	...
	transformed_source_.paint_uniform_color([1, 0, 0])
	if extra_data:
...
def test_one_epoch(device, model, test_loader, display=None):
	model.eval()
	test_loss = 0.0
	pred  = 0.0
	count = 0
...
def test(args, model, test_loader):
	test_loss = test_one_epoch(args.device, model, test_loader, display=True)
...
def options():
	...
	#parser.add_argument('--pretrained', default='checkpoints/EXP_1_PRNet/models/best_model.t7', type=str,
	#parser.add_argument('--pretrained', default='checkpoints/EXP_3_PRNet/models/best_model.t7', type=str,
	parser.add_argument('--pretrained', default='pretrained/exp_prnet/models/best_model.t7', type=str,
...
def main(own_data=False):
	...
	if not own_data:
		...
		#testset = StanfordFragments('PRNet')
	else:
