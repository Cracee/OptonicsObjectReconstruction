...
from dataset.data_loader import get_dataloader, get_datasets
...
def eval_model(
    model, dataloader, eval_epoch=None, metric_is_save=False, save_filetime="time"
):
    ...
    if eval_epoch is not None:
        if eval_epoch == -1:
            ...
            model_path = str(
                ...
                / "checkpoints"
                / "model_{:04}.pth".format(eval_epoch)
            )
    ...
    if metric_is_save:
        np.save(
            str(Path(cfg.OUTPUT_PATH) / ("eval_log_" + save_filetime + "_metric")),
            all_val_metrics_np,
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Point could registration evaluation code."
    )
    parser.add_argument(
        "--cfg",
        dest="cfg_file",
