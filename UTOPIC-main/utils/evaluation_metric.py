...
def matching_accuracy(pmat_pred, pmat_gt, ns):
    ...
    assert torch.all(
        (pmat_pred == 0) + (pmat_pred == 1)
    ), "pmat_pred can noly contain 0/1 elements."
    assert torch.all(
        (pmat_gt == 0) + (pmat_gt == 1)
    ), "pmat_gt should noly contain 0/1 elements."
    assert torch.all(torch.sum(pmat_pred, dim=-1) <= 1) and torch.all(
        torch.sum(pmat_pred, dim=-2) <= 1
    )
    assert torch.all(torch.sum(pmat_gt, dim=-1) <= 1) and torch.all(
        torch.sum(pmat_gt, dim=-2) <= 1
    )
...
def calcorrespondpc(pmat_pred, pc2_gt, ref_overlap):
    ...
    for i in range(pmat_pred.shape[0]):
        ...
        pmat_predi_index00 = torch.where(torch.sum(pmat_pred[i], dim=0) == 0)[
            0
        ]  # n row sum->1，1024
        pmat_predi_index01 = torch.where(torch.sum(pmat_pred[i], dim=1) == 0)[
            0
        ]  # n col sum->1024,1
        pc2[i, torch.cat((pmat_predi_index1[0], pmat_predi_index01))] = pc2_gt[
            i, torch.cat((pmat_predi_index1[1], pmat_predi_index00))
        ]
        overlap[i, torch.cat((pmat_predi_index1[0], pmat_predi_index01))] = ref_overlap[
            i, torch.cat((pmat_predi_index1[1], pmat_predi_index00))
        ]
        pmat_pred_index[i, pmat_predi_index1[0].cpu().numpy()] = (
            pmat_predi_index1[1].cpu().numpy()
        )
        pmat_pred_index[
            i, pmat_predi_index01.cpu().numpy()
        ] = pmat_predi_index00.cpu().numpy()
...
def compute_metrics(s_perm_mat, P1_gt, P2_gt, R_gt, T_gt, src_overlap, ref_overlap):
    ...
    concatenated = dcputil.concatenate(
        dcputil.inverse(R_gt.detach().cpu().numpy(), T_gt.detach().cpu().numpy()),
        np.concatenate(
    ...
    # Chamfer distance
    P1_transformed = torch.from_numpy(
        transform(
            torch.cat((R_pre, T_pre[:, :, None]), dim=2).detach().cpu().numpy(),
            P1_gt.detach().cpu().numpy(),
        )
    ).to(P1_gt)
    ...
    # Source distance
    P1_pre_trans = torch.from_numpy(
        transform(
            torch.cat((R_pre, T_pre[:, :, None]), dim=2).detach().cpu().numpy(),
            P1_gt.detach().cpu().numpy(),
        )
    ).to(P1_gt)
    P1_gt_trans = torch.from_numpy(
        transform(
            torch.cat((R_gt, T_gt[:, :, None]), dim=2).detach().cpu().numpy(),
            P1_gt.detach().cpu().numpy(),
        )
    ).to(P1_gt)
    ...
    dist_src = torch.min(
        torch.min(torch.sqrt(square_distance(P1_transformed, P2_gt)), dim=-1)[0],
        clip_val,
    )
    dist_ref = torch.min(
        torch.min(torch.sqrt(square_distance(P2_gt, P1_transformed)), dim=-1)[0],
        clip_val,
    )
    ...
    correspond_dis = torch.sqrt(
        torch.sum((P1_gt_trans_corr - P2_gt_copy_coor) ** 2, dim=-1, keepdim=True)
    )
    ...
    metrics = {
        ...
        "pre_transform": np.concatenate(
            (to_numpy(R_pre), to_numpy(T_pre)[:, :, None]), axis=2
        ),
        "gt_transform": np.concatenate(
            (to_numpy(R_gt), to_numpy(T_gt)[:, :, None]), axis=2
        ),
...
def summarize_metrics(metrics):
    """Summaries computed metrices by taking mean over all dataset instances"""
    summarized = {}
    for k in metrics:
        if k.endswith("mse"):
