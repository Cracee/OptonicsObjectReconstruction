...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        self, input_q, input_k, input_v, key_masks=None, attention_factors=None
    ):
        """
...
class AttentionLayer(nn.Module):
    ...
    def forward(
        self, input_states, memory_states, memory_masks=None, attention_factors=None
    ):
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, num_head, dropout=0.1, activation_fn="gelu", **kwargs):
        ...
        self.output = AttentionOutput(
            d_model, dropout=dropout, activation_fn=activation_fn, **kwargs
        )
    ...
    def __init__(self, blocks, d_model, num_head, dropout=0.1, activation_fn="gelu"):
        ...
        for block in self.blocks:
            if block not in ["self", "cross"]:
                raise ValueError(
                    'Unsupported block type "{}" in `VanillaConditionalTransformer`.'.format(
                        block
                    )
                )
            layers.append(
                TransformerLayer(
                    d_model, num_head, dropout=dropout, activation_fn=activation_fn
                )
            )
