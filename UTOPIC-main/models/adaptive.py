...
class AdaptiveConv(nn.Module):
    def __init__(self, k, in_channels, feat_channels, nhiddens, out_channels):
        ...
        self.conv1 = nn.Conv2d(
            nhiddens, nhiddens * in_channels, kernel_size=1, bias=False
        )
        ...
        self.linear = nn.Sequential(
            nn.Conv2d(nhiddens, out_channels, kernel_size=1, bias=False),
            nn.BatchNorm2d(out_channels),
        )
    ...
    def forward(self, points, feat, idx):
        ...
        x, _ = get_graph_feature(
            points, k=self.k, idx=idx
        )  # (bs, in_channels, num_points, k)
        y, _ = get_graph_feature(
            feat, k=self.k, idx=idx
        )  # (bs, feat_channels, num_points, k)
        ...
        kernel = kernel.permute(0, 2, 3, 1).view(
            batch_size, num_points, self.k, self.nhiddens, self.in_channels
        )  # (bs, num_points, k, nhiddens, in)
...
class AdaptiveFCN(nn.Module):
    ...
    def forward(self, x):
        ...
        x_edge = F.relu(self.bn(self.conv(x.unsqueeze(dim=-1)))).view(
