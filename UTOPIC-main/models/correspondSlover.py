...
def RANSACSVDslover(src_o, tgt_o, s_perm_mat):
    ...
    src_o0 = [
        src_o[i, list((weights_inlier[1][weights_inlier[0] == i]).cpu().numpy())]
        .cpu()
        .numpy()
        for i in range(s_perm_mat.shape[0])
    ]
    tgt_o0 = [
        tgt_o[i, list((weights_inlier[1][weights_inlier[0] == i]).cpu().numpy())]
        .cpu()
        .numpy()
        for i in range(s_perm_mat.shape[0])
    ]
    ...
    for i in range(len(src_o0)):
        ...
        corr = open3d.utility.Vector2iVector(
            np.arange(src_o0[i].shape[0])[:, None].repeat(2, axis=1)
        )
        reg_result = open3d.registration.registration_ransac_based_on_correspondence(
            src_o0i, tgt_o0i, corr, 0.2
        )
        R[i] = torch.from_numpy(reg_result.transformation[:3, :3]).to(s_perm_mat)
        t[i] = torch.from_numpy(reg_result.transformation[:3, 3])[:, None].to(
            s_perm_mat
        )
