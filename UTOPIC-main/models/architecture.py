...
def sinkhorn_rpm(
    log_alpha, n_iters: int = 5, slack: bool = True, eps: float = -1
) -> torch.Tensor:
...
class PipeLine(nn.Module):
    def __init__(self):
        ...
        self.transformer1 = TransformerModule(
            2 * self.embed_dim,
            self.embed_dim,
            self.embed_dim // 2,
            4,
        ...
        self.transformer2 = TransformerModule(
            self.embed_dim,
            self.embed_dim,
            self.embed_dim // 2,
            4,
    ...
    def getTriLength(self, points, pairs):
        ...
        temp = (
            (torch.arange(batch) * num)
            .reshape(batch, 1, 1, 1)
            .repeat(1, num, num_pair, 3)
            .cuda()
        )
        pairs = pairs + temp
        points = points.reshape(-1, 3)
        pairs = pairs.reshape(-1, num_pair, 3)
        tri_len[:, :, 0] = torch.sqrt(
            torch.sum(
                ((points[pairs[:, :, 0], :] - points[pairs[:, :, 1], :]) ** 2), dim=-1
            )
        )
        tri_len[:, :, 1] = torch.sqrt(
            torch.sum(
                ((points[pairs[:, :, 1], :] - points[pairs[:, :, 2], :]) ** 2), dim=-1
            )
        )
        tri_len[:, :, 2] = torch.sqrt(
            torch.sum(
                ((points[pairs[:, :, 0], :] - points[pairs[:, :, 2], :]) ** 2), dim=-1
            )
        )
    ...
    def get_geometric_structure(self, points):
        with torch.no_grad():
            ...
            dist_map = torch.sqrt(
                (torch.sum((points.unsqueeze(1) - points.unsqueeze(2)) ** 2, -1))
            )  # (B, N, N)
            dist_embedding = dist_map

            knn_indices = dist_map.topk(k=self.tri_k + 1, dim=2, largest=False)[
                1
            ]  # (B, N, k)
            ...
            knn_indices = knn_indices.unsqueeze(3).expand(
                batch_size, num_point, self.tri_k, 3
            )  # (B, N, k, 3)
            ...
            index = (
                index.repeat(batch_size, 1, self.tri_k * (self.tri_k - 1) // 2)
                .unsqueeze(-1)
                .cuda()
            )
            ...
            tri_len_map = torch.sum(
                torch.sqrt((tri_length.unsqueeze(1) - tri_length.unsqueeze(2)) ** 2),
                dim=-1,
            )
            tri_len_embedding = tri_len_map

        geo_embedding = torch.cat(
            [
                dist_embedding[..., None],
                angle_embedding[..., None],
                tri_len_embedding[..., None],
            ],
            dim=-1,
        )
    ...
    def forward(self, points_src, points_ref, phase):
        ...
        emb2_new = self.cross(
            torch.cat((emb_ref, torch.bmm(s.transpose(1, 2), emb_src)), dim=-1)
        )
        emb_src = emb1_new
        emb_ref = emb2_new
