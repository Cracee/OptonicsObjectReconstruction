...
from dataset.data_loader import get_datasets, get_dataloader
...
def train_eval_model(
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        print(
            "lr = "
            + ", ".join(["{:.2e}".format(x["lr"]) for x in optimizer.param_groups])
        )
        ...
        for inputs in dataloader["train"]:
            ...
            with torch.set_grad_enabled(True):
                ...
                overlap_pred = torch.cat(
                    (data_dict["src_overlap"], data_dict["ref_overlap"]), dim=1
                )
                overlap_gt = torch.cat((src_overlap_gt, ref_overlap_gt), dim=1)

                loss_item = overallLoss(
                    data_dict["s_pred"],
                    perm_mat,
                    num_src,
                    num_ref,
                    ...
                    points_src_raw,
                    points_ref_raw,
                    data_dict["coarse_src"],
                    data_dict["fine_src"],
                    data_dict["coarse_ref"],
                    data_dict["fine_ref"],
                    data_dict["prob"],
                )
                loss = (
                    loss_item["perm_loss"]
                    + loss_item["overlap_loss"]
                    + 1 * loss_item["c_s_cd_loss"]
                    + 1 * loss_item["c_r_cd_loss"]
                    + 1 * loss_item["f_s_cd_loss"]
                    + 1 * loss_item["f_r_cd_loss"]
                    + 0.5 * loss_item["overlap_prob_loss"]
                    + 0.1 * loss_item["kl_loss"]
                )
                ...
                s_perm_mat = lap_solver(
                    data_dict["s_pred"],
                    num_src,
                    num_ref,
                    data_dict["src_row_sum"],
                    data_dict["ref_col_sum"],
                )
                ...
                perform_metrics = compute_metrics(
                    ...
                    transform_gt[:, :3, :3],
                    transform_gt[:, :3, 3],
                    data_dict["src_overlap"],
                    data_dict["ref_overlap"],
                )
                ...
                all_train_metrics_np["perm_loss"].append(
                    np.repeat(loss_item["perm_loss"].item(), batch_cur_size)
                )
                all_train_metrics_np["overlap_loss"].append(
                    np.repeat(loss_item["overlap_loss"].item(), batch_cur_size)
                )
                all_train_metrics_np["c_s_cd_loss"].append(
                    np.repeat(loss_item["c_s_cd_loss"].item(), batch_cur_size)
                )
                all_train_metrics_np["f_s_cd_loss"].append(
                    np.repeat(loss_item["f_s_cd_loss"].item(), batch_cur_size)
                )
                all_train_metrics_np["c_r_cd_loss"].append(
                    np.repeat(loss_item["c_r_cd_loss"].item(), batch_cur_size)
                )
                all_train_metrics_np["f_r_cd_loss"].append(
                    np.repeat(loss_item["f_r_cd_loss"].item(), batch_cur_size)
                )
                all_train_metrics_np["overlap_prob_loss"].append(
                    np.repeat(loss_item["overlap_prob_loss"].item(), batch_cur_size)
                )
                all_train_metrics_np["kl_loss"].append(
                    np.repeat(loss_item["kl_loss"].item(), batch_cur_size)
                )

                if iter_num % cfg.STATISTIC_STEP == 0:
                    iter_log = (
                        "[{}] Epoch: [{:<3}/{:<3}] || Iteration: [{:<4}/{:<4}]".format(
                    ...
                    for k in all_train_metrics_np:
                        if k.endswith("loss") or k.startswith("acc"):
                            iter_log += (
                                ...
                                + ": {:.4f}".format(
                                    np.mean(
                                        np.concatenate(all_train_metrics_np[k])[
        ...
        all_train_metrics_np = {
            k: np.concatenate(all_train_metrics_np[k]) for k in all_train_metrics_np
        }
        ...
        writer.add_scalar(
            "train/err_r_deg_mean", summary_metrics["err_r_deg_mean"], epoch
        )
        ...
        if optimal_acc < val_metrics["acc_gt"]:
            ...
            save_best_acc_pth = str(
                checkpoint_path / "epoch_{}_model_best_acc.pth".format(epoch + 1)
            )
            torch.save(
                {
                    "epoch": epoch + 1,
        ...
        if optimal_rot > val_metrics["err_r_deg_mean"]:
            ...
            save_best_path = str(
                checkpoint_path / "epoch_{}_model_best.pth".format(epoch + 1)
            )
            torch.save(
                {
                    "epoch": epoch + 1,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Point could registration training & evaluation code."
    )
    parser.add_argument(
        "--cfg",
        dest="cfg_file",
        help="an optional config file",
        default="experiments/UTOPIC_Unseen_CropRPM_0.7_modelnet40.yaml",
        type=str,
    )
    ...
    if len(cfg.MODEL_NAME) != 0 and len(cfg.DATASET_NAME) != 0:
        out_path = get_output_dir(
            cfg.MODEL_NAME,
            cfg.DATASET_NAME
            + ("_Unseen_" if cfg.DATASET.UNSEEN else "_Seen_")
            + cfg.DATASET.NOISE_TYPE
            + ("_" + str(cfg.DATASET.PARTIAL_P_KEEP[0])),
        )
        cfg_from_list(["OUTPUT_PATH", out_path])
    assert (
        len(cfg.OUTPUT_PATH) != 0
    ), "Invalid OUTPUT_PATH! Make sure model name and dataset name are specified."
    if not Path(cfg.OUTPUT_PATH).exists():
        Path(cfg.OUTPUT_PATH).mkdir(parents=True)

    writer = SummaryWriter(
        log_dir=os.path.join(str(Path(cfg.OUTPUT_PATH)), "tensorboard")
    )
    ...
    dataloader = {
        x: get_dataloader(pc_dataset[x], shuffle=(x == "train"), phase=x)
        for x in ("train", "val")
    }
