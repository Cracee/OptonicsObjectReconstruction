...
class RandomTransformSE3:
    ...
    def __call__(self, sample):
        ...
        if "points" in sample:
            ...
            sample["points_src_raw"], _, _ = self.apply_transform(
                sample["points_raw"], transform_s_r
            )
    ...
    def __call__(self, sample):
        if "points" in sample:
            ...
            if "jitterflag" in sample or "cropflag" in sample:
                ...
                points_src_transform = se3.transform(
                    sample["transform_gt"], sample["points_src"][:, :3]
                )
                points_ref = sample["points_ref"][:, :3]
                dist_s2et, indx_s2et = nearest_neighbor(
                    points_src_transform, points_ref
                )
                dist_t2es, indx_t2es = nearest_neighbor(
                    points_ref, points_src_transform
                )
                padtype = 3
                padth = 0.05
                if padtype == 1:
                    for row_i in range(sample["points_src"].shape[0]):
                        if (
                            indx_t2es[indx_s2et[row_i]] == row_i
                    ...
                    if (
                        points_src_transform_rest.shape[0] > 0
                        and points_ref_rest.shape[0] > 0
                    ):
                        dist_s2et, indx_s2et = nearest_neighbor(
                        ...
                        for row_i in range(points_src_transform_rest.shape[0]):
                            if (
                                indx_t2es[indx_s2et[row_i]] == row_i
                                and dist_s2et[row_i] < padth * 2
                            ):
                                perm_mat[
                                    outlier_src_ind[row_i],
                                    outlier_ref_ind[indx_s2et[row_i]],
                                ] = 1
...
class PRNetTorchOverlapRatio:
    ...
    def jitter(self, pts):
        noise = np.clip(
            np.random.normal(0.0, scale=self.noise_std, size=(pts.shape[0], 3)),
            a_min=-self.clip,
            a_max=self.clip,
        )
    ...
    def knn(self, pts, random_pt, k1, k2):
        ...
        idx1 = distance.topk(k=k1, dim=0, largest=False)[
            1
    ...
    def __call__(self, sample):
        ...
        # pdb.set_trace()
        src_idx1, src_idx2 = self.knn(
            src, random_p1, k1=self.crop_num_points, k2=2048 - self.crop_num_points
        )
        ref_idx1, ref_idx2 = self.knn(
            ref, random_p1, k1=self.crop_num_points, k2=2048 - self.crop_num_points
        )
        ...
        overlap_idx = distance.topk(
            k=int(self.crop_num_points * self.overlap_ratio), dim=0, largest=False
        )[1]
        k1_points = ref[ref_idx1, :][overlap_idx, :]
        distance = torch.sum((ref[ref_idx2, :] - k2) ** 2, dim=1)
        nonoverlap_idx = distance.topk(
            k=self.crop_num_points - int(self.crop_num_points * self.overlap_ratio),
            dim=0,
            largest=False,
        )[1]
        ...
        sample["points_ref_raw"], _ = self.apply_transform(
            sample["points_raw"], transform_s_r
        )
