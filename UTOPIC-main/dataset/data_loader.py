...
def load_data(partition):
    ...
    for h5_name in sorted(
        ...
        data = np.concatenate([f["dataset"][:], f["normal"][:]], axis=-1)
    ...
    def __getitem__(self, item):
        ...
        transform_igt = np.concatenate(
            (
                transform_gt[:, :3].T,
                np.expand_dims(
                    -(transform_gt[:, :3].T).dot(transform_gt[:, 3]), axis=1
                ),
            ),
            axis=-1,
        )
        ...
        ret_dict = {
            "points": [
                torch.Tensor(x) for x in [sample["points_src"], sample["points_ref"]]
            ],
            ...
            "transform_gt": [
                torch.Tensor(x)
                for x in [
                    transform_gt.astype("float32"),
                    transform_igt.astype("float32"),
                ]
            ],
            "overlap_gt": [
                torch.Tensor(x)
                for x in [sample["src_overlap_gt"], sample["ref_overlap_gt"]]
            ],
...
def get_datasets(
    ...
    if cfg.DATASET_NAME == "ModelNet40":
        transforms = get_transforms(
            ...
            rot_mag=rot_mag,
            trans_mag=trans_mag,
            partial_p_keep=partial_p_keep,
        )
        transforms = torchvision.transforms.Compose(transforms)
        datasets = ModelNet40(
            partition, unseen, transforms, crossval=crossval, train_part=train_part
        )
    else:
