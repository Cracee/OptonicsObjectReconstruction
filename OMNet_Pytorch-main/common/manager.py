...
class Manager:
    ...
    def summarize_metric_status(self, metrics, split):
        if split == "val":
            for k in metrics:
                if k.endswith("MSE"):
                    self.val_status[k[:-3] + "RMSE"].set(
                        val=np.sqrt(self.val_status[k].avg)
                    )
                else:
                    continue
        elif split == "test":
            for k in metrics:
                if k.endswith("MSE"):
                    self.test_status[k[:-3] + "RMSE"].set(
                        val=np.sqrt(self.test_status[k].avg)
                    )
                else:
                    continue
        else:
    ...
    def print_train_info(self):
        ...
        print_str = "{} Epoch: {:4d}, lr={:.4f} ".format(
    ...
    def print_metrics(self, split, title="Eval", color="red", only_best=False):
        ...
        print_str = " | ".join(
            "{}: {:4g}".format(k, v.avg) for k, v in metric_status.items()
        )
        if only_best:
            if is_best:
                self.logger.info(
                    colored(
                        "Best Epoch: {}, {} Results: {}".format(
                            self.epoch, title, print_str
                        ),
                        color,
                        attrs=["bold"],
                    )
            ...
            self.logger.info(
                colored(
                    "Epoch: {}, {} Results: {}".format(self.epoch, title, print_str),
                    color,
                    attrs=["bold"],
                )
            )
    ...
    def write_loss_to_tb(self, split):
        for k, v in self.loss_status.items():
            if split == "train":
                self.local_loss_writter.add_scalar(
                    "train_Loss/{}".format(k), v.val, self.step
                )
            elif split == "val":
                self.local_loss_writter.add_scalar(
                    "val_Loss/{}".format(k), v.val, self.step
                )
            elif split == "test":
                self.local_loss_writter.add_scalar(
                    "test_Loss/{}".format(k), v.val, self.step
                )
            else:
                raise ValueError("Wrong split string: {}".format(split))
    ...
    def write_metric_to_tb(self, split):
        if split == "val":
            for k, v in self.val_status.items():
                self.local_metric_writter.add_scalar(
                    "val_Metric/{}".format(k), v.avg, self.epoch
                )
        elif split == "test":
            for k, v in self.test_status.items():
                self.local_metric_writter.add_scalar(
                    "test_Metric/{}".format(k), v.avg, self.epoch
                )
        else:
            raise ValueError("Wrong split string: {}".format(split))
    ...
    def check_best_save_last_checkpoints(self, save_latest_freq=5, save_best_after=50):
        ...
        if self.dataloaders["val"] is not None:
            ...
            if is_best:
                ...
                self.logger.info(
                    "Current is val best, score={:.7f}".format(self.best_val_score)
                )
                # save checkpoint
        ...
        if self.dataloaders["test"] is not None:
            ...
            if is_best:
                ...
                self.logger.info(
                    "Current is test best, score={:.7f}".format(self.best_test_score)
                )
                # save checkpoint
                if self.epoch > save_best_after:
                    best_ckpt_name = os.path.join(
