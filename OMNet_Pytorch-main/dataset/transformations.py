        ...
        if self.mode == "hdf":
            ...
            if sample["num_points"] == -1:
                ...
                if points_src.shape[0] > sample["num_points"]:
                    sample["points_src"] = points_src[
                        np.random.choice(
                            points_src.shape[0], sample["num_points"], replace=False
                        ),
                        :,
                    ]
                else:
                    rand_idxs = np.concatenate(
                        [
                            np.random.choice(
                                points_src.shape[0], points_src.shape[0], replace=False
                            ),
                            np.random.choice(
                                points_src.shape[0],
                                sample["num_points"] - points_src.shape[0],
                                replace=True,
                            ),
                        ]
                    )
                ...
                if points_ref.shape[0] > sample["num_points"]:
                    sample["points_ref"] = points_ref[
                        np.random.choice(
                            points_ref.shape[0], sample["num_points"], replace=False
                        ),
                        :,
                    ]
                else:
                    rand_idxs = np.concatenate(
                        [
                            np.random.choice(
                                points_ref.shape[0], points_ref.shape[0], replace=False
                            ),
                            np.random.choice(
                                points_ref.shape[0],
                                sample["num_points"] - points_ref.shape[0],
                                replace=True,
                            ),
                        ]
                    )
            ...
            points_src_fps = (
                gather_operation(
                    points_src_flipped, furthest_point_sample(points_src, 4096)
                )
                .transpose(1, 2)
                .contiguous()
                .detach()
            ...
            points_ref_fps = (
                gather_operation(
                    points_ref_flipped, furthest_point_sample(points_ref, 4096)
                )
                .transpose(1, 2)
                .contiguous()
                .detach()
...
def fetch_transform(params):
    ...
    _logger.info(
        "Val and Test transforms: {}".format(
